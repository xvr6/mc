ServerEvents.generateData("after_mods", (event) => {
  ...
  let monPowers = data.starterMons.map((x) => {
    return {
      ...
      entity_action_gained: {
        ...
        command:
          'kubejs persistent-data entity @s merge {"monName":"' + x.name + '"}',
      },
      entity_action_lost: {
        type: "origins:execute_command",
  ...
  let monJson = {
    ...
    name: "Cobblemon",
    description:
      "You've elected to start as a cobblemon.  Pick what you're gonna start as.",
    origins: [
      {
        condition: {
          type: "origins:origin",
  ...
  for (const i in data.regions) {
    ...
    regionPowers.push({
      ...
      entity_action_gained: {
        type: "origins:execute_command",
        command:
          'kubejs persistent-data entity @s merge {"monRegion":' +
          (parseInt(i) + 1) +
          "}",
      },
      entity_action_lost: {
  ...
  regionLayerList.push({
    ...
    origins: ["startmon:regular", "startmon:off"],
  });
